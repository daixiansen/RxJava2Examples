...
import com.example.retrofitlibrary.interceptor.CacheInterceptor;
...
public class SingleRxHttp {
    ...
    public SingleRxHttp addHeaders(Map<String, String> headerMaps) {
    ...
    public OkHttpClient.Builder getSingleOkHttpBuilder() {
        ...
        singleOkHttpBuilder.readTimeout(readTimeout > 0 ? readTimeout : READ_TIME_OUT, TimeUnit.MILLISECONDS);

        singleOkHttpBuilder.writeTimeout(writeTimeout > 0 ? writeTimeout : WRITE_TIME_OUT, TimeUnit.MILLISECONDS);

        singleOkHttpBuilder.connectTimeout(connectTimeout > 0 ? CONNECT_TIME_OUT : 10, TimeUnit.MILLISECONDS);
